...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    lazy val leftElem = left.mostRetweeted
    lazy val rightElem = right.mostRetweeted

    val lRet = if(!left.empty) leftElem.retweets else 0
    val rRet = if(!right.empty) rightElem.retweets else 0

    if (elem.retweets > lRet && elem.retweets > rRet) elem
    else if (lRet > rRet) leftElem
    else rightElem
  }
